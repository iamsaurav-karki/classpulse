  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.user.role === 'teacher' && !response.user.isVerified) {
        toast.success('Welcome back! Your teacher account is pending verification.', { duration: 5000 });
      } else {
        toast.success(`Welcome back, ${response.user.name}!`, { duration: 3000 });
      }
    ...
    <div className="min-h-screen bg-green-50 flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full">
        ...
        <div className="text-center mb-8">
          ...
          <h2 className="text-3xl font-bold text-gray-900 mt-4">Sign in to your account</h2>
          <p className="mt-2 text-gray-600">
            Don't have an account?{' '}
            <Link href="/auth/register" className="font-medium text-green-600 hover:text-green-700 transition-colors">
